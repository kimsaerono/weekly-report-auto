...
for (const chat of chatsData.data.items) {
  ...
  const msgData = await msgRes.json() as { code: number; data?: { items?: Array<{ body?: { content?: string }; msg_type?: string; create_time?: string }> } }
      ...
      if (msg.msg_type === 'text') {
        try {
          ...
          if (content && content.length > 3) {
            ...
            allMessages.push({ time, chatName, content })
          }
        } catch {}
      }
    }
