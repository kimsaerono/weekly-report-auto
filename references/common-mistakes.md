...
| 新项目/新仓库不进周报 | 引擎里没有该项目的业务映射（旧版写死 `REPO_TO_DOMAIN`） | 通用引擎已删掉全部业务映射，项目名（仓库名）直出，无需配置改动 |
...
- 归类改为**数据驱动**：一级 = 仓库名/项目名直出；git 类型（feat/fix/refactor/docs…）映射动词前缀；分类取自模板；无任何业务写死配置。新增代码/文档无需改本工具。